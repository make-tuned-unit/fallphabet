-- Fallphabet Daily Challenge Fix for Supabase
-- Run this SQL in your Supabase SQL Editor to fix the function conflicts

-- First, drop any existing functions that might conflict
DROP FUNCTION IF EXISTS has_daily_attempt_today(VARCHAR);
DROP FUNCTION IF EXISTS submit_daily_challenge_score(VARCHAR, INTEGER, INTEGER, VARCHAR, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS get_daily_challenge_leaderboard();
DROP FUNCTION IF EXISTS get_daily_seed();

-- Add new columns to the existing leaderboard table (if they don't exist)
ALTER TABLE leaderboard 
ADD COLUMN IF NOT EXISTS attempt_date DATE DEFAULT CURRENT_DATE,
ADD COLUMN IF NOT EXISTS daily_seed INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS top_word VARCHAR(20),
ADD COLUMN IF NOT EXISTS top_word_score INTEGER DEFAULT 0;
...
DROP INDEX IF EXISTS idx_daily_challenge_unique;
...
DROP INDEX IF EXISTS idx_daily_challenge_date;
...
BEGIN
  ...
  IF (SELECT has_attempted FROM has_daily_attempt_today(player_name_param)) THEN
